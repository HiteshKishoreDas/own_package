...
class Tools(object):
    ...
    def deriv(self, Y, X=None):
        ...
        **Outputs:**
        ...
        DfDx = (
            np.roll(Yarr, 1) * (x12 / (x01 * x02))
            + Yarr * (1.0 / x12 - 1.0 / x01)
            - np.roll(Yarr, -1) * (x01 / (x02 * x12))
        )
        # Formulae for the first and last points:

        DfDx[0] = (
            Yarr[0] * (x01[1] + x02[1]) / (x01[1] * x02[1])
            - Yarr[1] * x02[1] / (x01[1] * x12[1])
            + Yarr[2] * x01[1] / (x02[1] * x12[1])
        )
        DfDx[n - 1] = (
            -Yarr[n - 3] * x12[n2] / (x01[n2] * x02[n2])
            + Yarr[n - 2] * x02[n2] / (x01[n2] * x12[n2])
            - Yarr[n - 1] * (x02[n2] + x12[n2]) / (x02[n2] * x12[n2])
        )
        ...
        **Outputs:**
        ...
        if geometry == "spherical":
            ...
            for j in range(n2):
                dV1[:, j] = A1 * dx1
            dV2 = (
                np.meshgrid(x1, np.sin(x2) * dx2)[0].T
                * np.meshgrid(x1, np.sin(x2) * dx2)[1].T
            )
        ...
        for i in range(1, n1 - 1):
            du1[i, :] = (
                0.5 * (A1[i + 1] * u1[i + 1, :] - A1[i - 1] * u1[i - 1, :]) / dV1[i, :]
            )
        for j in range(1, n2 - 1):
            du2[:, j] = (
                0.5 * (A2[j + 1] * u2[:, j + 1] - A2[j - 1] * u2[:, j - 1]) / dV2[:, j]
            )
    ...
    def congrid(self, a, newdims, method="linear", centre=False, minusone=False):
        ...
        **Outputs:**
        ...
        if method == "neighbour":
            ...
            print(
                "Congrid error: Unrecognized interpolation type. Currently only 'neighbour', 'nearest','linear' and 'spline' are supported."
            )
            return None
    ...
    def sph2cyl(self, D, Dx, rphi=None, theta0=None):
        ...
        **Usage:**
        ...
        F = R.copy()
        F.flat[n] = w1.flat[n] * (
            w3.flat[n] * Dx.flat[NN1.flat[n]] + w4.flat[n] * Dx.flat[NN3.flat[n]]
        ) + w2.flat[n] * (
            w3.flat[n] * Dx.flat[NN2.flat[n]] + w4.flat[n] * Dx.flat[NN4.flat[n]]
        )
