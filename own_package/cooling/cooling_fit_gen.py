...
if __name__=="__main__":
    ...
    # plt.plot(T_k,Lam_fit_arr[k]*(T_k/T_k[0])**alp_fit_arr[k],linewidth=5)

    plt.plot(T_arr,Lam_arr,linestyle='dashed',linewidth=2,\
        color='k',label="Cooling function")
    ...
    for i_t,t in enumerate(T_breaks[:-1]):
        ...
        T_k = np.logspace(np.log10(T_breaks[i_t]),np.log10(T_breaks[i_t+1]),num=bin_n)

        plt.plot(T_k, lam0_fit[i_t]*(T_k/T_breaks[i_t])**alp_fit[i_t])

    # print(T_breaks)

    # plt.plot(T_breaks,pwlf_lam,linewidth=5,zorder=-2,color='tab:red',\
    #     label='PWLF fit with '+str(N_bin)+' segments')


    plt.title("Piecewise power law fit for cooling function using PWLF",fontsize=titlesize)
    plt.xlabel("T (K)",fontsize=labelsize)
    plt.ylabel(r"$\Lambda$ (T)",fontsize=labelsize)
    plt.legend(fontsize=legendsize)
    plt.tick_params(labelsize=ticksize)

    # plt.savefig("power_law_fit_pwlf.png")
