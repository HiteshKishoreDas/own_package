...
def clump_finder(arr, arr_cut, method="scipy", above_cut=False):
    ...
    if method == "scipy":
        ...
        print(
            'clump_analysis.py::clump_finder(): Invalid "method", using default("scipy")... '
        )
        return clump_finder_scipy(arr, arr_cut, above_cut=False)
...
def clump_covariance(label_arr, only_cov=False):
    if __debug__:
        print(
            f"clump_analysis.py::clump_covariance(): clump_covariance called with {only_cov = }..."
        )
...
def clump_size(clump_num, label_arr, clump_centering=False):
    ...
    if np.isnan(np.sum(cov_dict["cov"])):
        print(
            "clump_analysis.py::clump_size(): Insufficient number of cells (need >1) in the clump for covariance calculation..."
        )
...
def clump_length(clump_num, label_arr, k_n=1, n_jobs=1, skip_data=1):
    ...
    adj_mat = (
        (
            np.abs(
                skm.pairwise_distances(
                    data[::skip_data, 0].reshape(-1, 1) / skip_data, n_jobs=n_jobs
                )
            )
            <= k_n
        )
        & (
            np.abs(
                skm.pairwise_distances(
                    data[::skip_data, 1].reshape(-1, 1) / skip_data, n_jobs=n_jobs
                )
            )
            <= k_n
        )
        & (
            np.abs(
                skm.pairwise_distances(
                    data[::skip_data, 2].reshape(-1, 1) / skip_data, n_jobs=n_jobs
                )
            )
            <= k_n
        )
...
def clump_length_test(clump_num, label_arr, k_n=1, n_jobs=1, skip_data=1):
    """"""
    label_clump = clump_select(clump_num, label_arr)
    data = (clump_flatten(label_clump).astype(float)).T
    ...
    a = time.time()
    adj_mat_full = adjacency_matrix(data, k_n=k_n, n_jobs=n_jobs)
    adj_mat_skip = adjacency_matrix(data, skip_data=skip_data, k_n=k_n, n_jobs=n_jobs)

    np.fill_diagonal(adj_mat_full, False)
    np.fill_diagonal(adj_mat_skip, False)

    b = time.time()
    print(f"Calculating adjucency matrix: {b-a} s")

    a = time.time()
    graph_nx_full = nx.from_numpy_array(adj_mat_full.astype(int))
    graph_nx_skip = nx.from_numpy_array(adj_mat_skip.astype(int))
    b = time.time()
    print(f"Creating graph: {b-a} s")

    a = time.time()
    path_length = dict(nx.all_pairs_shortest_path_length(graph_nx_skip))
    b = time.time()
    print(f"Calculating path lengths: {b-a} s")
    ...
    max_path_length_full = nx.dijkstra_path_length(
        graph_nx_full, max_path_pair[0] * skip_data, max_path_pair[1] * skip_data
    )
    ...
    if __name__ == "__main__":
        ...
        return_dict["max_path_pair"] = max_path_pair

        return_dict["graph_nx_skip"] = graph_nx_skip
        return_dict["graph_nx_full"] = graph_nx_full

        return_dict["max_path_length_skip"] = max_path_length * skip_data
        return_dict["max_path_length_full"] = max_path_length_full
...
if __name__ == "__main__":
    ...
    def alpha_plot(c_arr, log_flag=False):
        return pt.poly_alpha(
            c_arr, log_flag=log_flag, order=1, cut=0
        )  # ,cut=np.sqrt(frac_aniso.min()*frac_aniso.max()))
    ...
    fig, ax, sc = pt.render_scatter_3d(
        inp_arr=label_arr_select, alpha_fn=alpha_plot, cmap="Paired"
    )
    plt.show()
    ...
    max_path_length_skip = return_dict["max_path_length_skip"]
    max_path_length_full = return_dict["max_path_length_full"]

    k1, k2 = return_dict["max_path_pair"]

    graph_nx_skip = return_dict["graph_nx_skip"]
    graph_nx_full = return_dict["graph_nx_full"]

    max_path_skip = nx.dijkstra_path(graph_nx_skip, k1, k2)
    max_path_skip = np.array(max_path_skip) * skip_data
    max_coord_skip = data[max_path_skip]

    max_path_full = nx.dijkstra_path(graph_nx_full, k1 * skip_data, k2 * skip_data)
    max_path_full = np.array(max_path_full)
    max_coord_full = data[max_path_full]
