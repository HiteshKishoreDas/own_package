...
def clump_elongation_hist(
    ...
    with open(
        f"{file_path}/save_arr/clump_size/{sim_list[file_ind]}clump_Tcut_{int(Tcut)}_Nsnap_{N_snap}.pkl",
        "rb",
    ) as f:
    ...
    bar_args0 = dict(
        bar_args,
        **{
            "color": f"C{file_ind%2 + 4}",
            "alpha": 0.3,
            "label": bar_args["label"]
            + r": $N_{\rm blob} = $"
            + f"{clump_dict['n_blob']}",
        },
    )
    fig, ax = ph.plot_histogram_1d(
        elongation,
        norm=norm,
        bins=bin_elongation_arr,
        fig=fig,
        ax=ax,
        new_fig=False,
        bar_args=bar_args0,
...
def clump_elongation_percentile(fig, ax, file_ind=0, volume_cut=0.0, **kwargs):
    ...
    hst_0 = ph.plot_histogram_1d(
        elongation[0], bins=bin_elongation_arr, return_hist=True, with_fig=False
    )
    hst_1 = ph.plot_histogram_1d(
        elongation[1], bins=bin_elongation_arr, return_hist=True, with_fig=False
    )
    ...
    ax.plot(
        hst_0[1][bin_a:-1],
        np.cumsum(hst_0[0][bin_a:]) / np.sum(hst_0[0][bin_a:]),
        label="HD ",
        **kwargs,
    )
    ax.plot(
        hst_1[1][bin_a:-1],
        np.cumsum(hst_1[0][bin_a:]) / np.sum(hst_1[0][bin_a:]),
        label="MHD",
        **kwargs,
    )
...
def clump_length_hist(
    ...
    clump_length = np.array(clump_dict["clump_dict"]) + 1  # *dx_lsh_list[file_ind]

    norm = np.sum(clump_length > length_cut)
    ...
    # percentile_edge_a = hst[1][bin_idx]
    # percentile_edge_b = hst[1][bin_idx + 1]
    ...
    ax.axvline(
        ...
        linestyle="dashed",
        color=bar_color,
        label=f"{bar_label} {percentile} %ile",
    )
    ...
    if return_list:
        return ax, clump_length[clump_length > length_cut]
    else:
        return ax
...
def clump_size_hist(
    ...
    bar_args0 = dict(
        bar_args,
        **{"color": f"C1", "alpha": 0.2, "label": bar_args["label"] + ": Shortest"},
    )
    fig, ax = ph.plot_histogram_1d(
        clump_list_0,
        bins=bin_arr,
    ...
    bar_args1 = dict(
        bar_args,
        **{"color": f"C2", "alpha": 0.4, "label": bar_args["label"] + ": Intermediate"},
    )
    fig, ax = ph.plot_histogram_1d(
        clump_list_1,
        bins=bin_arr,
    ...
    bar_args2 = dict(
        bar_args,
        **{"color": f"C3", "alpha": 0.6, "label": bar_args["label"] + ": Longest"},
    )
    fig, ax = ph.plot_histogram_1d(
        clump_list_2,
        bins=bin_arr,
    ...
    if __name__ == "__main__":
        ...
        leg.add_legend(
            [
                {
                    "linestyle": "solid",
                    "label": r"$N_{clump} = $" + f'{clump_dict["n_blob"]}',
                }
            ],
            ax=ax,
            legend_loc="upper left",
...
def clump_elongation_volume_2D_hist(
    ...
    elongation = clump_elongation_calculation(
        file_ind, volume_cut=volume_cut, pair_flag=False
    )
...
if __name__ == "__main__":
    ...
    ax.set_xlabel("Longest filament length (in gridcells)")
    ax.set_ylabel(r"Frequency/$N_{\rm cloud}$")
